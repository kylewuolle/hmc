...
import (
	...
	"github.com/K0rdent/kcm/internal/controller/ipam/adapters"
)
...
var _ = Describe("ClusterIPAMClaim Controller", func() {
	createIPAMClaim := func(resourceName, namespace string) kcm.ClusterIPAMClaim {
		...
		return kcm.ClusterIPAMClaim{
			...
			Spec: kcm.ClusterIPAMClaimSpec{
				Provider:             adapters.IPAMInclusterAdapterName,
	...
	Context("When reconciling a resource", func() {
		...
		It("should successfully reconcile the resource", func() {
			...
			Expect(clusterIPAM.Spec.Provider).To(Equal(adapters.IPAMInclusterAdapterName))
		})
	})
})
